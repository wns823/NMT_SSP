...
class TransformerLecaModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        transformer_leca_architecture(args)
        ...
        encoder = cls.build_encoder(args, src_dict, encoder_embed_tokens, task=task)
        decoder = cls.build_decoder(args, tgt_dict, decoder_embed_tokens)
        encoder.get_decoder(decoder)
    ...
    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens, task=None):
    ...
    def get_span_targets(self, sample, net_output):
        """Get targets from either the sample or the net's output."""
        return sample["span_output"]
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, task=None):
        ...
        if self.consnmt:
            self.cons_pos_embed = ConsPosiEmb(args.decoder_embed_dim, self.padding_idx, dictionary.index('<sep>'))
    ...
    def forward(
        ...
        if not self.consnmt or (self.decoder.sep_id not in src_tokens.view(-1)):
            x = encoder_embedding = self.embed_scale * self.embed_tokens(src_tokens)   # x = embed = self.embed_scale * token_embedding
            x += self.embed_positions(src_tokens)
            if self.consnmt and (self.seg_embed is not None):
                x += self.seg_embed(torch.zeros_like(src_tokens))
        else:

            sep_position = min((src_tokens== self.decoder.sep_id).nonzero()[:,1])
            ...
            cons_x =  self.embed_scale * self.decoder.embed_tokens(cons_sent)
            cons_posi_x = self.cons_pos_embed(cons_sent)
            seg_cons = torch.cumsum((cons_sent== self.decoder.sep_id),dim=1).type_as(cons_sent)
            seg_cons[(cons_sent== self.decoder.pad_id)] = torch.tensor([16]).type_as(seg_cons)        
            cons_seg_emb=self.seg_embed(seg_cons)   

            encoder_embedding = torch.cat((src_x, cons_x),dim=1)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.padding_idx = embed_tokens.padding_idx
        self.sep_id = dictionary.index('<sep>')
    ...
    def forward(
        ...
        if self.use_ptrnet: # {"attn": [attn], "inner_states": inner_states}
            src_tokens = encoder_out['src_tokens'][0] if encoder_out is not None else None 
            src_tokens = src_tokens.unsqueeze(1).expand(extra['attn'][0].size())
            src_masks = src_tokens.eq(self.eos_id) | src_tokens.eq(self.pad_id) | src_tokens.eq(self.sep_id)

            dec_enc_attn = extra['attn'][0].masked_fill(src_masks, float(1e-15))  ##bsz x tgtlen x srclen
            enc_hids = encoder_out['encoder_out'][0].transpose(0,1) ### srclen x bsz x hidsize  -> bsz x srclen x hidsize
            ctx = torch.bmm(dec_enc_attn, enc_hids)  ## bsz x tgtlen x hidsize
            gate = self.ptrnet(ctx, extra["inner_states"][-1].transpose(0,1)) ## [0]
        else:
            gate, src_tokens, dec_enc_attn = None, None, None 


        extra['dec_enc_attn'] = dec_enc_attn
        extra['gate'] = gate
        extra['src_tokens'] = src_tokens
...
def transformer_leca_architecture(args):
